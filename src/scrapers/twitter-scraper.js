...
class TwitterScraper {
    constructor(options = {}) {
        ...
        this.originalTweetUrl = null; // Store original tweet URL for navigation
    }
    ...
    async initialize() {
        try {
            ...
            await this.page.route('**/*.{png,jpg,jpeg,gif,webp,svg,ico,woff,woff2,ttf,eot}', route => route.abort());
    ...
    async navigateToTweet(tweetUrl) {
        try {
            ...
            this.originalTweetUrl = tweetUrl; // Store for later navigation
            ...
            try {
                ...
                logger.warn('Article element not found, checking for alternative selectors');
            ...
            if (error.message.includes('Timeout')) {
                ...
                try {
                    ...
                    logger.error('Alternative navigation also failed:', retryError.message);
                }
            }
            throw error;
        }
    ...
    async loadAuthentication() {
        try {
            if (this.cookiesFile) {
                ...
                logger.warn('No authentication provided - may not be able to access all content');
            }
        } catch (error) {
    ...
    async verifyAuthentication() {
        try {
            ...
            // Check if we're logged in by looking for user menu or login button
            const userMenu = await this.page.$('[data-testid="SideNav_AccountSwitcher_Button"]');
            const loginButton = await this.page.$('[data-testid="loginButton"]');
            
            if (userMenu) {
                // Try to get username from the page
                const usernameElement = await this.page.$('[data-testid="SideNav_AccountSwitcher_Button"] [dir="ltr"]');
                const username = usernameElement ? await usernameElement.textContent() : 'Unknown';
                logger.info(`✓ Authentication successful - logged in as: ${username}`);
                return true;
            } else if (loginButton) {
                logger.error('✗ Authentication failed - not logged in (login button visible)');
                return false;
            } else {
                // Check for any indication we're on the login page
                const isLoginPage = await this.page.$('input[name="text"]'); // Username input on login page
                if (isLoginPage) {
    ...
    async extractRetweeters() {
        try {
            ...
            // Try multiple selector strategies for retweet button
            let retweetButton = await this.page.$('[data-testid="retweet"]');
            if (!retweetButton) {
                retweetButton = await this.page.$('[data-testid="unretweet"]');
            }
            if (!retweetButton) {
                retweetButton = await this.page.$('div[role="button"][aria-label*="Retweet"]');
            }
            if (!retweetButton) {
                // Try finding by SVG icon
                retweetButton = await this.page.$('div[role="button"]:has(svg[viewBox="0 0 24 24"])');
            }
            
            if (!retweetButton) {
                logger.warn('Retweet button not found - trying alternative approach');
                
                // Try looking for retweet count text directly
                const retweetCountElement = await this.page.$('span:has-text("retweet")').catch(() => null);
            ...
            // Get retweet count from button or nearby text
            let retweetCount = 0;
            try {
                if (retweetButton) {
                    ...
                    if (retweetCount === 0) {
                        // Check if there's a count in aria-label
            ...
            logger.info(`Found ${retweetCount} retweets, attempting to access retweet list`);

            // Try clicking the retweet button
            if (retweetButton) {
                await retweetButton.click();
                await this.page.waitForTimeout(2000);

                // Look for "View Retweets" or similar option in the dropdown
                const viewRetweetsOption = await this.page.$('div[role="menuitem"]:has-text("View retweets")').catch(() => null);
                ...
                // Alternative: direct navigation to retweets page
                const originalUrl = this.page.url();
                const tweetId = originalUrl.match(/status\/(\d+)/)?.[1];
                if (tweetId) {
                    const retweetsUrl = originalUrl.replace(/\/status\/\d+.*/, `/status/${tweetId}/retweets`);
                    ...
                    // Navigate back to original tweet for next extraction
            ...
            logger.warn('Could not access retweet list - may require authentication or tweet may be private');
            return [];
        } catch (error) {
    ...
    async extractLikers() {
        try {
            ...
            if (!likeButton) {
                likeButton = await this.page.$('div[role="button"][aria-label*="Like"]');
            }
            if (!likeButton) {
                likeButton = await this.page.$('div[role="button"][aria-label*="like"]');
            }
            ...
            if (!likeButton) {
                ...
                // Try looking for like count text directly
                const likeCountElement = await this.page.$('span:has-text("like")').catch(() => null);
                if (likeCountElement) {
            ...
            // Get like count from button or nearby text
            let likeCount = 0;
            try {
                if (likeButton) {
            ...
            if (likeButton) {
                ...
                // Look for "Liked by" or similar text to find the likes page
                const likedByLink = await this.page.$('a[href*="/likes"]').catch(() => null);
                if (likedByLink) {
                ...
                // Alternative: direct navigation to likes page
                const originalUrl = this.page.url();
                const tweetId = originalUrl.match(/status\/(\d+)/)?.[1];
                if (tweetId) {
                    const likesUrl = originalUrl.replace(/\/status\/\d+.*/, `/status/${tweetId}/likes`);
            ...
            logger.warn('Could not access like list - may require authentication or tweet may be private');
            return [];
        } catch (error) {
    ...
    async scrollAndExtractUsers(maxScrolls = 50) {
        ...
        while (scrollCount < maxScrolls) {
            // No timeout for extraction - let it run as long as needed
            const userElements = await this.page.$$('[data-testid="UserCell"]').catch(() => []);
            ...
            for (const element of userElements) {
                try {
                    ...
                    // Strategy 1: Look for href in direct child link
                    ...
                    // Strategy 2: Look for username in data-testid="UserName"
                    if (!username) {
                        const userNameElement = await element.$('[data-testid="UserName"]');
                        if (userNameElement) {
                            // Look for the username span (usually contains @)
                            const usernameSpan = await userNameElement.$('div:nth-child(2) span');
                            if (usernameSpan) {
                    ...
                    // Strategy 3: Look for any element with @ prefix
                    if (!username) {
            ...
            logger.info(`[${elapsedSeconds}s] Scroll ${scrollCount + 1}: Found ${users.size} unique users (+${newUsersThisScroll} new)`);
            ...
            // Check if we're still getting new users
            if (users.size === previousUsersCount) {
                noNewUsersCount++;
                if (noNewUsersCount >= 3) {
                    logger.info('No new users found in last 3 scrolls, stopping extraction');
                    break;
                }
            } else {
                noNewUsersCount = 0;
        ...
        logger.info(`Extraction complete: ${users.size} users found in ${totalTime} seconds`);
    ...
    async extractFollowers(username) {
        try {
            ...
            // Use x.com domain instead of twitter.com
            ...
            if (!isAccessible) {
                logger.warn('Cannot access followers list - may require authentication or be private');
                return [];
            }

            // Look for followers count first
    ...
    async checkFollowersAccessible() {
        try {
            ...
            // Check for "This account's posts are protected" message
            const protectedText = await this.page.$(':has-text("This account")').catch(() => null);
            if (protectedText) {
    ...
    async getFollowersCount(username) {
        try {
            // Look for followers link/count in navigation
            const followersLink = await this.page.$(`a[href="/${username}/followers"]`);
            if (followersLink) {
    ...
    async debugPageState(description = 'debug') {
        try {
            ...
            logger.info(`Elements found - Tweet: ${!!tweet}, Retweet: ${!!retweetBtn}, Like: ${!!likeBtn}`);
