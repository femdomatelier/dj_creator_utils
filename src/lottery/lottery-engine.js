...
class LotteryEngine {
    ...
    drawWinners(participants, numberOfWinners) {
        ...
        if (numberOfWinners > participants.length && !this.allowDuplicateWins) {
            throw new Error(`Cannot select ${numberOfWinners} winners from ${participants.length} participants`);
        }
    ...
    weightedDraw(participants, numberOfWinners) {
        ...
        for (let i = 0; i < numberOfWinners; i++) {
            ...
            } while (usedIndices.has(randomIndex) && usedIndices.size < weightedPool.length);
    ...
    validateDraw(result, participants, numberOfWinners) {
        ...
        if (uniqueWinners.size !== result.winners.length && !this.allowDuplicateWins) {
        ...
        for (const winner of result.winners) {
            if (!participantUsernames.has(winner.username)) {
                return { valid: false, reason: `Winner ${winner.username} not in participant list` };
            }
        }
