...
program
    .name('x-lottery')
    .description('Twitter/X Lottery System - Draw winners from tweet interactions')
    .version('1.0.0');
...
program
    ...
    .option('--timeout <ms>', 'Navigation timeout in milliseconds', parseInt, 60000)
    ...
    .action(async (options) => {
        ...
        if (options.config) {
            try {
                ...
                if (options.winners && options.winners !== 1) config.winners = options.winners;
                if (options.output && options.output !== 'text') config.output = options.output;
                ...
                if (process.argv.includes('--require-retweet')) config.requireRetweet = true;
                if (process.argv.includes('--require-like')) config.requireLike = true;
                if (process.argv.includes('--require-follow')) config.requireFollow = true;
            ...
            // No config file, use CLI options with defaults
            config = { ...options };
        }

        // Validate required fields
        if (!config.url) {
            console.error(chalk.red('Error: Tweet URL is required (use -u or specify in config file)'));
            process.exit(1);
        }
        ...
        try {
            ...
            // Auto-determine what data to collect based on filter requirements
            const needRetweets = config.requireRetweet;
            const needLikes = config.requireLike;  
            const needFollowers = config.requireFollow;
            
            // If no filters are specified, default to retweets only
            if (!needRetweets && !needLikes && !needFollowers) {
                config.retweets = true;
                config.requireRetweet = false; // Don't filter, just collect
                outputManager.outputWarning('No filters specified, collecting retweets only');
            } else {
                // Auto-collect based on filter requirements
                config.retweets = needRetweets;
                config.likes = needLikes;
                config.followers = needFollowers;
                outputManager.outputInfo(`Auto-collection: retweets=${needRetweets}, likes=${needLikes}, followers=${needFollowers}`);
            }
            ...
            // Debug configuration
            if (config.verbose) {
                outputManager.outputInfo(`Configuration - Retweets: ${config.retweets}, Likes: ${config.likes}, Followers: ${config.followers}`);
                outputManager.outputInfo(`Filters - RequireRetweet: ${config.requireRetweet}, RequireFollow: ${config.requireFollow}`);
            }

            // Check for configuration inconsistencies
            if (config.requireRetweet && !config.retweets) {
                outputManager.outputWarning('Configuration inconsistency: requireRetweet is true but retweets collection is disabled');
            }
            if (config.requireFollow && !config.followers) {
                outputManager.outputWarning('Configuration inconsistency: requireFollow is true but followers collection is disabled');
            }
            if (config.requireLike && !config.likes) {
                outputManager.outputWarning('Configuration inconsistency: requireLike is true but likes collection is disabled');
            }

            // Progress callback for real-time updates
            const progressCallback = config.verbose ? (progress) => {
                if (progress.type === 'user_found') {
                    console.log(chalk.gray(`  → Found user: @${progress.username} (Total: ${progress.total})`));
                } else if (progress.type === 'scroll_progress' && progress.scrollCount % 5 === 0) {
                    console.log(chalk.yellow(`  📊 Progress: ${progress.totalUsers} users after ${progress.elapsedTime}s`));
                }
            } : null;
            ...
            if (config.authToken || config.csrfToken || config.cookiesFile) {
                ...
                if (!isAuthenticated) {
                    outputManager.outputWarning('Authentication verification failed - proceeding anyway');
                }
            }
            ...
            while (!navigationSuccess && retryCount <= maxRetries) {
                try {
                    if (retryCount > 0) {
                        outputManager.outputInfo(`Retry attempt ${retryCount}/${maxRetries}`);
                    }
                    await scraper.navigateToTweet(config.url);
                    navigationSuccess = true;
                    
                    // Debug screenshot after successful navigation
                    if (config.debug) {
                    ...
                    if (retryCount > maxRetries) {
                        // Debug screenshot on final failure
                        if (config.debug) {
                            await scraper.debugPageState('navigation-failed');
                        }
                        throw navError;
                    }
                    outputManager.outputWarning(`Navigation failed, retrying... (${retryCount}/${maxRetries})`);
            ...
            // Extract tweet author first if needed
            if (config.followers) {
            ...
            if (config.followers && tweetAuthor) {
                ...
                // Debug screenshot before followers extraction
                if (config.debug) {
                    await scraper.debugPageState('before-followers-extraction');
                }
                
                followers = await scraper.extractFollowers(tweetAuthor);
                outputManager.outputInfo(`Found ${followers.length} followers of @${tweetAuthor}`);
                
                // Debug screenshot after followers extraction
            ...
            if (config.exclude) {
                filters.excludeUsernames = config.exclude.split(',').map(u => u.trim());
                outputManager.outputInfo(`Excluding: ${filters.excludeUsernames.join(', ')}`);
            }
            ...
            outputManager.outputSuccess(`${participants.length} participants qualified for lottery`);
            ...
            const validation = lotteryEngine.validateDraw(result, participants, config.winners);
            if (!validation.valid) {
            ...
            const outputData = {
                result,
                statistics,
                participants, // Include all participants for file output
                metadata: {
                    tweetUrl: config.url,
