class DataParser {
  ...
  applyFilters(users) {
    ...
    if (this.filters.requireLike) {
      filteredUsers = filteredUsers.filter(
        (user) =>
          (user.types && user.types.includes("like")) || user.type === "like",
      );
    }

    if (
      this.filters.excludeUsernames &&
      Array.isArray(this.filters.excludeUsernames)
    ) {
      const excludeSet = new Set(
        this.filters.excludeUsernames.map((u) => u.toLowerCase()),
      );
      filteredUsers = filteredUsers.filter(
        (user) => !excludeSet.has(user.username.toLowerCase()),
      );
    }

    if (
      this.filters.includeUsernames &&
      Array.isArray(this.filters.includeUsernames)
    ) {
      const includeSet = new Set(
        this.filters.includeUsernames.map((u) => u.toLowerCase()),
      );
